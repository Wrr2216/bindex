...
/**
 * Whether Bluetooth room presence counts toward placement: the BLE feature is
 * switched on, which itself needs tracking. The configuration is cached and
 * refreshed on every settings change, so this is cheap to ask per batch.
 */
export async function bleAvailable(): Promise<boolean> {
  return (await getConfig()).features.ble;
}
